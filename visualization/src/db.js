...
async function initializeDatabase() {
  const [sqlite3, response] = await Promise.all([sqlite3InitModule(), fetch(databaseUrl())]);
  if (!response.ok) {
  ...
  try {
    ...
    const result = sqlite3.capi.sqlite3_deserialize(db.pointer, "main", pointer, bytes.byteLength, bytes.byteLength, sqlite3.capi.SQLITE_DESERIALIZE_FREEONCLOSE);
    db.checkRc(result);
...
function buildRule(rule, columns = RULE_COLUMNS) {
  ...
  const values = [...new Set([...ruleValues(rule?.value), ...ruleValues(rule?.listValue)])];

  if (!column || !values.length || !["is", "contains", "in"].includes(matcher) || !["include", "exclude"].includes(mode)) {
    return null;
  }
  if (matcher === "contains") {
    return [`${column} ${mode === "exclude" ? "NOT LIKE" : "LIKE"} ?`, [`%${values[0]}%`]];
  }
  if (matcher === "is") {
    return [`${column} ${mode === "exclude" ? "<>" : "="} ?`, [values[0]]];
  }

  const placeholders = values.map(() => "?").join(", ");
  return [`${column} ${mode === "exclude" ? "NOT IN" : "IN"} (${placeholders})`, values];
}
...
function queryParts(filters = {}) {
  ...
  for (const rule of filters.rules ?? []) {
    ...
    combinedRule = combinedRule === null ? predicate : `(${combinedRule} ${rule.join === "OR" ? "OR" : "AND"} ${predicate})`;
  ...
  const cutoffColumn = isJee ? (isRankMode ? "ai24.Merit_Rank" : "ai24.Percentile") : isRankMode ? "cc24.Merit_Rank" : "cc24.Percentile";
  ...
  const sortMap = isJee ? (isRankMode ? JEE_RANK_SORT_COLUMNS : JEE_SORT_COLUMNS) : isRankMode ? RANK_SORT_COLUMNS : SORT_COLUMNS;
...
export async function getExplorerResults(filters = {}) {
  ...
  const groupBy = isJee ? "GROUP BY ci.College_Code, ci.College_Name, bi.Branch_Code, bi.Branch_Name, bi.Home_University, bi.Status, ai24.CAP_Round" : "";
  const countSql = isJee ? `SELECT COUNT(*) AS total FROM (SELECT 1 ${fromWhere} ${groupBy})` : `SELECT COUNT(*) AS total ${fromWhere}`;
  const total = Number(selectScalar(db, countSql, parameters) ?? 0);
  const offset = (page - 1) * pageSize;
  const rows = selectRows(db, `SELECT ${selectColumns(exam)} ${fromWhere} ${groupBy} ${orderBy} LIMIT ? OFFSET ?`, [...parameters, pageSize, offset]);
...
export async function getCollegeDetails(collegeCode, payload = {}) {
  ...
  for (const row of rows) {
    if (numericScore != null && row[cutoffKey] != null) {
      row.fitDifference = isRankMode ? Number(row[cutoffKey] - numericScore) : Number((numericScore - row[cutoffKey]).toFixed(2));
    } else {
...
export async function getPredictions(profile = {}) {
  ...
  const db = await database();
  const cutoffColumn = exam === "JEE" ? (isRankMode ? "ai24.Merit_Rank" : "ai24.Percentile") : isRankMode ? "cc24.Merit_Rank" : "cc24.Percentile";

  const cutoffKey = isRankMode ? (exam === "JEE" ? "jeeRank2024" : "cetRank2024") : exam === "JEE" ? "jee2024" : "cet2024";
  ...
  for (const bucket of buckets) {
    ...
    const groupBy = isJee ? "GROUP BY ci.College_Code, ci.College_Name, bi.Branch_Code, bi.Branch_Name, bi.Home_University, bi.Status, ai24.CAP_Round" : "";
    const queryParams = bucket.orderParams ? [...parameters, ...bucket.orderParams, 200] : [...parameters, 200];
    const rows = selectRows(db, `SELECT ${selectColumns(exam)} ${fromWhere} ${groupBy} ${orderBy} LIMIT ?`, queryParams);
