...
export default function App() {
  ...
  const selectTrend = async (row) => { setSelected(row); setTab('trends'); setTrendLoading(true); try { const categoryParam = row.category ? `&category=${encodeURIComponent(row.category)}` : ''; setTrendData(await requestJson(`/api/trends?collegeCode=${row.collegeCode}&branchCode=${encodeURIComponent(row.branchCode)}${categoryParam}&capRound=${row.capRound}`)) } catch (e) { setError(e.message); setTrendData({ cet: [], jee: [] }) } finally { setTrendLoading(false) } }
