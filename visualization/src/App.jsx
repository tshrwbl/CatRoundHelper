...
function Explorer({ rows, loading, onSelectTrend, onOpenCollege, pagination, sort, onSort, percentiles, exam }) {
  ...
  return <><div className="table-wrap"><table>
    <thead><tr>{heading('College / branch', 'collegeName')}{exam !== 'JEE' && heading('Category', 'category')}{exam !== 'JEE' && heading('CET 2024', 'cet2024')}{heading('JEE 2024', 'jee2024')}{heading(`3-year ${exam} change`, changeKey)}<th /></tr></thead>
    <tbody>{rows.map((row, index) => {
      ...
      return <tr key={`${row.collegeCode}-${row.branchCode}-${row.category || ''}-${row.capRound}-${index}`}>
        ...
        {exam !== 'JEE' && <PercentileCell cutoff={row.cet2024} score={percentiles.CET} rank={row.cetRank2024} />}<PercentileCell cutoff={row.jee2024} score={percentiles.JEE} rank={row.jeeRank2024} />
...
function Trends({ selected, data, loading }) {
  ...
  return <section className="trend-view"><div><p className="eyebrow">Selected option</p><h2>{selected.collegeName}</h2><p>{selected.branchName}{selected.category ? ` · ${selected.category}` : ''}</p></div>
    {loading ? <div className="empty"><LoaderCircle className="spin" /> Loading history…</div> : <div className="chart"><div className="chart-key"><span className="cet-key">CET percentile</span><span className="jee-key">JEE percentile</span></div><ResponsiveContainer width="100%" height={310}><RechartsLineChart data={chartData} margin={{ top: 10, right: 24, left: -12, bottom: 0 }}>
      <CartesianGrid strokeDasharray="3 3" stroke="var(--chart-grid)" /><XAxis dataKey="year" stroke="var(--chart-axis)" /><YAxis domain={['auto', 'auto']} stroke="var(--chart-axis)" />
      <Tooltip contentStyle={{ background: 'var(--tooltip-bg)', border: '1px solid var(--tooltip-border)', borderRadius: 12 }} labelStyle={{ color: 'var(--text-color)' }} itemStyle={{ color: 'var(--text-color)' }} /><Line type="monotone" dataKey="cet" name="CET percentile" stroke="#a78bfa" strokeWidth={3} dot={{ r: 5 }} connectNulls /><Line type="monotone" dataKey="jee" name="JEE percentile" stroke="#2dd4bf" strokeWidth={3} dot={{ r: 5 }} connectNulls />
...
function CollegePage({ college, data, loading, collegeRules, onEditFilters, percentile, percentiles, exam, collegeOptions, onSelectCollege }) {
  ...
  return <section className="college-page"><div className="college-header"><div><p className="eyebrow">College intelligence</p><h2>{college.collegeName}</h2><p>Code {college.collegeCode} · 2024 {exam === 'JEE' ? 'branch' : 'category-wise'} cutoff overview</p></div><div className="college-controls"><CollegePicker options={collegeOptions} college={college} onSelect={onSelectCollege} /><button className="open-filter-builder" onClick={onEditFilters}><SlidersHorizontal size={16} /> College filters{collegeRules.length ? ` (${collegeRules.length})` : ''}</button><p>Your {exam} percentile: <strong>{percentile || '—'}%</strong></p></div></div><div className="college-metrics"><article><span>Branches shown</span><strong>{new Set(latest.map((row) => row.branchCode)).size}</strong></article><article><span>{exam === 'JEE' ? 'Branch cutoffs' : 'Category cutoffs'}</span><strong>{latest.length}</strong></article><article><span>Your score</span><strong>{percentile || '—'}%</strong></article></div><div className="college-chart"><div><h3>Most competitive branches</h3><p>Highest 2024 {exam} cutoff across each branch’s categories.</p></div>{chartData.length ? <ResponsiveContainer width="100%" height={290}><RechartsBarChart data={chartData} layout="vertical" margin={{ left: 14, right: 30 }}><CartesianGrid strokeDasharray="3 3" stroke="var(--chart-grid)" /><XAxis type="number" stroke="var(--chart-axis)" domain={['auto', 'auto']} /><YAxis type="category" dataKey="branch" width={185} stroke="var(--text-color)" tick={{ fontSize: 11 }} /><Tooltip contentStyle={{ background: 'var(--tooltip-bg)', border: '1px solid var(--tooltip-border)', borderRadius: 12 }} labelStyle={{ color: 'var(--text-color)' }} itemStyle={{ color: 'var(--text-color)' }} /><Bar dataKey="cutoff" name={`${exam} percentile`} fill="#8b7cf6" radius={[0, 5, 5, 0]} /></RechartsBarChart></ResponsiveContainer> : <div className="empty compact">No 2024 cutoff data for this exam and filter set.</div>}</div><div className="college-chart"><div><h3>College cutoff movement</h3><p>Average category-wise percentile across the selected branches.</p></div><ResponsiveContainer width="100%" height={260}><RechartsLineChart data={historyData} margin={{ top: 10, right: 24, left: -12 }}><CartesianGrid strokeDasharray="3 3" stroke="var(--chart-grid)" /><XAxis dataKey="year" stroke="var(--chart-axis)" /><YAxis domain={['auto', 'auto']} stroke="var(--chart-axis)" /><Tooltip contentStyle={{ background: 'var(--tooltip-bg)', border: '1px solid var(--tooltip-border)', borderRadius: 12 }} labelStyle={{ color: 'var(--text-color)' }} itemStyle={{ color: 'var(--text-color)' }} /><Legend /><Line type="monotone" dataKey="cet" name="CET average" stroke="#a78bfa" strokeWidth={3} connectNulls /><Line type="monotone" dataKey="jee" name="JEE average" stroke="#2dd4bf" strokeWidth={3} connectNulls /></RechartsLineChart></ResponsiveContainer></div><div className="college-table"><h3>{exam === 'JEE' ? 'All 2024 branch cutoffs' : 'All 2024 category-wise cutoffs'}</h3><div className="table-wrap"><table><thead><tr><th>Branch</th><th>Branch status</th>{exam !== 'JEE' && <th>Category</th>}{exam !== 'JEE' && collegeHeading('CET', 'cetPercentile')}{collegeHeading('JEE', 'jeePercentile')}</tr></thead><tbody>{sortedLatest.map((row, index) => <tr key={`${row.branchCode}-${row.category || ''}-${row.capRound}-${index}`}><td><strong>{row.branchName}</strong><span>{row.branchCode}</span></td><td>{row.branchStatus || '—'}</td>{exam !== 'JEE' && <td><span className="tag">{row.category}</span></td>}{exam !== 'JEE' && <PercentileCell cutoff={row.cetPercentile} score={percentiles.CET} rank={row.cetRank} />}<PercentileCell cutoff={row.jeePercentile} score={percentiles.JEE} rank={row.jeeRank} /></tr>)}</tbody></table></div></div></section>
}

function Predictor({ filters, percentiles }) {
  ...
  const runPrediction = async (event) => { event.preventDefault(); setLoading(true); setError(''); try { setResult(await getPredictions({ ...filters, percentile })) } catch (requestError) { setError(requestError.message) } finally { setLoading(false) } }
...
export default function App() {
  ...
  useEffect(() => { if (!college) return undefined; let ignored = false; setCollegeLoading(true); getCollegeDetails(college.collegeCode, { rules: collegeRules, exam: filters.exam, percentile: candidatePercentiles[filters.exam], applyActiveFilters: true }).then((payload) => { if (!ignored) setCollegeData(payload) }).catch((e) => { if (!ignored) setError(e.message) }).finally(() => { if (!ignored) setCollegeLoading(false) }); return () => { ignored = true } }, [college, collegeRules, filters.exam, candidatePercentiles])
